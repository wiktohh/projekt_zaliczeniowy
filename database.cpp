...
void Database::changePassword() {
	...
	string sql = "UPDATE passwords SET password=? WHERE (id=? and user=?);";

	sqlite3_stmt* stmt;
	const char* ozTest;

	sqlite3_prepare_v2(db, sql.c_str(), sql.length(), &stmt, &ozTest);
	sqlite3_bind_text(stmt, 1, newPassword.c_str(), newPassword.length(), NULL);
...
void Database::createAccount() {
	...
	sqlite3_stmt* stmt2;
	const char* ozTest2;
	string valid = "select * from users where login=?; ";
	sqlite3_prepare_v2(db, valid.c_str(), valid.length(), &stmt2, &ozTest2);
	sqlite3_bind_text(stmt2, 1, login.c_str(), login.length(), NULL);
	int rows_count = 0;
